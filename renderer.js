...
function _renderBayesianWeights() {
  ...
  const labels = {
    ...
    betfair_coherence: "Betfair-Kohärenz",
  };
...
function _loadCrossSport() {
  ...
  fetch('poly_cross_sport.json?t=' + Date.now())
    ...
    .then(j => { window.POLY_CROSS_SPORT = j || { discrepancies: [] }; _crossSportLoading = false;
                 ...
                 // eigener Poly-Radar-Tab: Panel nachziehen, sobald die Daten da sind
                 const prp = (typeof document !== 'undefined') && document.getElementById('polyRadarPanel');
                 if (prp && prp.style.display !== 'none' && typeof _renderPolyRadar === 'function') prp.innerHTML = _renderPolyRadar(); });
}
...
// ── Poly Radar (eigener Menüpunkt, 22.07.2026, Lucas: „alle Poly-Sport-Märkte an einem Ort") ──
// Vereint Fußball/MLS (Poly vs Pinnacle aus unseren Fixture-Daten) + die 2-Wege-Sportarten
// (US-Sport, Kampfsport …) in EINEM filterbaren Board, sortiert nach Konvergenz. Liest dieselbe
// poly_cross_sport.json — der Backend-Lauf mischt Fußball jetzt mit hinein.
const _POLY_RADAR_SPORTMETA = {
  soccer_mls:  { label: 'MLS',  emoji: '⚽', cat: 'fussball', catLabel: '⚽ Fußball' },
  soccer_liga: { label: 'Top-5', emoji: '⚽', cat: 'fussball', catLabel: '⚽ Fußball' },
  soccer_wm:   { label: 'WM',   emoji: '⚽', cat: 'fussball', catLabel: '⚽ Fußball' },
  basketball_nba:        { label: 'NBA', emoji: '🏀', cat: 'ussport', catLabel: '🏀 US-Sport' },
  americanfootball_nfl:  { label: 'NFL', emoji: '🏈', cat: 'ussport', catLabel: '🏀 US-Sport' },
  baseball_mlb:          { label: 'MLB', emoji: '⚾', cat: 'ussport', catLabel: '🏀 US-Sport' },
  icehockey_nhl:         { label: 'NHL', emoji: '🏒', cat: 'ussport', catLabel: '🏀 US-Sport' },
  mma_mixed_martial_arts:{ label: 'MMA', emoji: '🥊', cat: 'kampf',   catLabel: '🥊 Kampfsport' },
  boxing_boxing:         { label: 'Boxen', emoji: '🥊', cat: 'kampf', catLabel: '🥊 Kampfsport' },
  tennis:                { label: 'Tennis', emoji: '🎾', cat: 'tennis', catLabel: '🎾 Tennis' },
};
function _prSportMeta(sk) {
  return _POLY_RADAR_SPORTMETA[sk] || { label: sk || '?', emoji: '🎯', cat: 'sonst', catLabel: '🎯 Sonstige' };
}
let _polyRadarCat = 'all';   // aktiver Kategorie-Filter
function _setPolyRadarCat(cat) {
  _polyRadarCat = cat;
  const p = document.getElementById('polyRadarPanel');
  if (p) p.innerHTML = _renderPolyRadar();
}

function _renderPolyRadar() {
  if (!window.POLY_CROSS_SPORT && !_crossSportLoading) _loadCrossSport();
  if (_crossSportLoading && !window.POLY_CROSS_SPORT) {
    return `<div style="max-width:1000px;margin:0 auto;padding:48px 0;text-align:center;color:var(--muted);font-size:13px;">🎯 Poly-Radar-Daten werden geladen …</div>`;
  }
  const rep = window.POLY_CROSS_SPORT || {};
  let disc = (rep.discrepancies || []).slice();
  const tr = rep.trackRecord || {};

  // Kategorie-Chips aus den tatsächlich vorhandenen Sportarten ableiten.
  const catsPresent = [];
  const seenCats = new Set();
  disc.forEach(d => { const c = _prSportMeta(d.sport).cat; if (!seenCats.has(c)) { seenCats.add(c); catsPresent.push(c); } });
  const catLabelFor = {};
  disc.forEach(d => { const m = _prSportMeta(d.sport); catLabelFor[m.cat] = m.catLabel; });

  const chip = (cat, label, active) => `<button onclick="_setPolyRadarCat('${cat}')" style="padding:6px 14px;border-radius:20px;border:1px solid ${active ? '#a78bfa' : 'var(--border)'};background:${active ? 'rgba(167,139,250,.16)' : 'transparent'};color:${active ? '#c4b5fd' : 'var(--muted)'};font-size:12px;font-weight:${active ? 700 : 500};cursor:pointer;">${label}</button>`;
  const chips = [chip('all', 'Alle', _polyRadarCat === 'all')]
    .concat(catsPresent.map(c => chip(c, catLabelFor[c] || c, _polyRadarCat === c))).join(' ');

  if (_polyRadarCat !== 'all') disc = disc.filter(d => _prSportMeta(d.sport).cat === _polyRadarCat);

  // Sortierung: konvergierende Lücken zuerst (das echte Signal), dann nach Lückengröße.
  disc.sort((a, b) => {
    const ca = a.convergePP == null ? -999 : a.convergePP, cb = b.convergePP == null ? -999 : b.convergePP;
    if ((cb > 0.5) !== (ca > 0.5)) return (cb > 0.5 ? 1 : 0) - (ca > 0.5 ? 1 : 0);
    return Math.abs(b.gapPP) - Math.abs(a.gapPP);
  });

  const intro = `<div style="max-width:1000px;margin:0 auto 14px;padding:12px 16px;background:rgba(167,139,250,.06);border-left:3px solid #a78bfa;font-size:12px;color:var(--muted);line-height:1.6;border-radius:0 8px 8px 0;">
    <b style="color:#a78bfa">Wo liegt das Geld auf Polymarket „falscher"?</b> — Poly-Preis gegen die de-viggte scharfe Pinnacle, über alle Sportarten. Eine große Lücke ist nur ein <b>Kandidat</b>; echt wird sie erst, wenn sie über die Tage <b>konvergiert</b> (Poly läuft zur Pinnacle). Read-only, kein Trade.</div>`;

  // Track-Record-Zeile (Ehrlichkeit + späteres Verkaufsargument).
  const trLine = (tr.tracked > 0)
    ? `<div style="max-width:1000px;margin:0 auto 12px;display:flex;gap:10px;align-items:center;font-size:12px;color:var(--muted);">
        <span style="padding:5px 12px;background:rgba(63,185,80,.1);border-radius:8px;color:#3fb950;font-weight:700;">${tr.converged}/${tr.tracked} konvergiert</span>
        <span>von allen je verfolgten Lücken hat sich die Poly-Seite Richtung Pinnacle bewegt — der Beleg, dass die Meldung echt war.</span></div>`
    : '';

  const chipBar = `<div style="max-width:1000px;margin:0 auto 14px;display:flex;gap:8px;flex-wrap:wrap;">${chips}</div>`;

  if (!disc.length) {
    const seen = rep.polyRowsSeen || 0, matched = rep.matched || 0;
    const body = (matched > 0 || seen > 0)
      ? `✅ Aktuell <b>keine Lücke ≥6pp</b> in dieser Auswahl.<br><span style="font-size:11px;opacity:.85;"><b>${matched}</b> Märkte gegen die scharfe Pinnacle verglichen. Poly und Pinnacle liegen eng beieinander — auf liquiden Märkten der Normalfall. Eine echte Fehlbepreisung erscheint hier, sobald Poly spürbar abweicht.</span>`
      : `🎯 Noch keine Daten.<br><span style="font-size:11px;opacity:.8;">Der Radar läuft am Mac-Runner (Poly ist EU-geoblockt) und füllt sich beim nächsten Lauf.</span>`;
    return intro + trLine + chipBar + `<div style="max-width:1000px;margin:0 auto;padding:44px 24px;text-align:center;color:var(--muted);font-size:13px;line-height:1.6;">${body}</div>`;
  }

  const rows = disc.slice(0, 40).map(d => {
    const m = _prSportMeta(d.sport);
    const gapCol = Math.abs(d.gapPP) >= 10 ? '#f85149' : Math.abs(d.gapPP) >= 7 ? '#e3b341' : '#8b949e';
    const conv = d.convergePP;
    const convCell = (conv == null)
      ? `<span style="color:var(--muted);font-style:italic;">neu</span>`
      : conv > 0.5
        ...
        : conv < -0.5
          ...
          : `<span style="color:var(--muted);" title="Lücke steht — Artefakt-Verdacht">→ 0</span>`;
    return `<tr style="border-top:1px solid var(--border);">
      <td style="padding:9px 12px;font-size:12px;white-space:nowrap;">${m.emoji} <span style="color:var(--muted);">${m.label}</span></td>
      <td style="padding:9px 12px;font-size:13px;">${d.event || ''} · <span style="color:var(--muted)">${d.outcome || ''}</span></td>
      <td style="padding:9px 12px;text-align:right;font-size:12px;color:#a78bfa;">${d.polyPP}%</td>
      <td style="padding:9px 12px;text-align:right;font-size:12px;color:#5eead4;">${d.pinnPP}%</td>
      <td style="padding:9px 12px;text-align:right;font-weight:700;color:${gapCol};">${d.gapPP > 0 ? '+' : ''}${d.gapPP}pp</td>
      <td style="padding:9px 12px;text-align:right;font-size:12px;">${convCell}</td>
    </tr>`;
  }).join('');

  return intro + trLine + chipBar + `<div style="max-width:1000px;margin:0 auto;background:var(--card);border:1px solid var(--border);border-radius:12px;overflow:hidden;">
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr style="background:rgba(255,255,255,0.02);">
        <th style="padding:10px 12px;text-align:left;font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;">Sport</th>
        <th style="padding:10px 12px;text-align:left;font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;">Markt</th>
...
if (typeof window !== 'undefined') { window._renderPolyRadar = _renderPolyRadar; window._setPolyRadarCat = _setPolyRadarCat; }

...
**Stand:** 07.09.2026 abends (Übersicht-Checkup: vier Funde — Money-Map-Fehljoin, „verliert" ohne Verlustbeleg, stiller Ranglisten-Filter, drei falsch benannte Zahlen; davor: raw-first als Funktion statt als Konvention; „Über der Norm" statt „Auffällig"; zweite Achse in der Spielklasse-Liste. Davor: Stake-Spielklasse + die drei Wege, auf denen ein fertiges Feature leer aussieht; §1 Poly-/Wallets-Zeilen sind Stand 24.08.)
...
19. **Eine kurze Liste ohne Erklärung liest sich wie ein Defekt** (07.09.2026, Betfair-Terminal): Das Terminal zeigt `betfair_consensus.games` — und das sind **4 von 125 Spielen** im Feed, weil `qualifies_radar` eine Liquiditäts-Schwelle zieht (größter FT-Markt ≥ €20K in Top-Ligen bzw. €15K sonst, HT ≥ €10K/€5K). Die Schwelle ist richtig, ihr Fehlen im Text nicht: eine Vier-Zeilen-Tabelle ohne Grund sieht kaputt aus, mit Grund ist sie ein Ergebnis. → Die Fußzeile nennt jetzt beide Zahlen, die Schwelle und wohin die übrigen Spiele gehen (Live-Radar). Dazu die `× Liga-Norm`-Spalte (dieselbe `_normRatio`-Rechnung wie die Live-Liste, kein zweiter Rechenweg) und die **gemessene** Richtung aus `betfair_direction.json` statt der aus der Kurve geschätzten — der Produzent liefert sie je Markt und je Runner, das Terminal hat sie sich selbst zusammengereimt.

18. **Eine Zahl ohne ihren Bezug ist Dekoration** (07.09.2026, Lucas über das Poly-Terminal: *„es war halt nichtssagend und ich konnte damit nichts anfangen"*): Das Whale-Tape zeigte `$50.000`. Für die eine Wallet ist das Dienstag, für die andere der größte Einsatz ihres Lebens — und genau diese Unterscheidung lag seit Wochen fertig in `poly_wallet_norm.json` (2.057 Wallets mit Median/p90/Max), wurde aber von keiner Fläche gelesen. → Jede Whale-Zeile trägt jetzt `× eigene Norm`, die Terminal-Tabelle den stärksten Faktor je Seite (`⟂` für die Gegenseite), und der Drawer eine Für-/Gegen-Bilanz, in der jede Zeile ihre Basis nennt. Regel: **ein Betrag ohne Bezugsgröße gehört nicht auf eine Entscheidungsfläche — entweder es gibt eine gelernte Norm, oder es steht „neu" da.**

16. **Ein Beleg braucht die passende Grenze** (07.09.2026, Übersicht-Checkup): `betfair_track_record` fällte beide Urteile an derselben Grenze — „trägt" bei UNTERgrenze > 0 (richtig) und „verliert" bei UNTERgrenze ≤ −10 % (falsch). Eine tiefe Untergrenze belegt **Unsicherheit**, keinen Verlust. Gemessen: 40 Buckets mit „verliert", **37 mit Obergrenze über null**, 18 mit positivem Punktschätzer, Spitzenwert **ROI +32,6 % bei „verliert"**. Das Urteil entfernte Zeilen aus der Übersicht-Rangliste und mutete Terminal-Zeilen. → `roiOg` wird mitgeschrieben; „verliert" verlangt OG < 0; die alte Schwelle heißt jetzt `fade` (Risiko-Marke) und trägt weiter alle Gates. Regel: **folgen belegt die Untergrenze über null, dagegenhalten die Obergrenze unter null — dieselbe Regel wie in `stake_liga_stufe.kreuz`.**

17. **Ein Kandidaten-Pool ohne Zeitbezug macht jeden Namens-Matcher zum Glücksspiel** (07.09.2026): `match_poly` suchte gegen den GANZEN Close-Pool — 2.494 von 2.557 Einträgen sind abgerechnete Snapshots (absichtlich 30 Tage gehalten, für die Auswertung). So jointe ein libanesisches Pokalspiel auf einen sechs Tage alten Saudi-Markt und zeigte $267.964 fremdes Geld als „Konsens 3/3". Je größer der Pool, desto sicherer der Zufallstreffer. → Wer **vor dem Anpfiff dieses Spiels** abgerechnet wurde, ist kein Kandidat. Regel: **ein Matcher ist nur so gut wie sein Pool — Namen prüfen kommt nach Zeit prüfen.**

15. **Ein Sicherheitsnetz, das zu grob greift, ist ein Leck** (07.09.2026): `pages_ballast.py` löscht Wurzel-JSONs, die keine Frontend-Datei erwähnt — und liess, um dynamisch gebaute Namen (`ds + '_poly_prices.json'`) nicht zu verlieren, **jedes** Namens-Endstück ab einem Unterstrich gelten. Generische Endstücke (`ledger.json`, `_results.json`, `_cache.json`) treffen aber praktisch immer: `stake_bet_ledger.json` (15,4 MB) fuhr im Deploy mit, weil irgendwo `liga_signal_ledger.json` steht. Gemessen **34,1 MB blinde Passagiere**. → Ein Endstück zählt nur noch direkt hinter einer echten Verkettungsgrenze (`'`, `"`, `` ` ``, `}`) — genau dort, wo ein dynamischer Name im Code anfängt. Artefakt 160,3 → 126,3 MB, Budget 160 → 140. Regel: **ein Netz gegen Fehlalarm muss so eng sein wie der Fall, den es fängt — sonst fängt es alles und meldet nie.**
...
20. **Ein Altersmarker ist Identität, kein Rauschen** (08.09.2026, Übersicht-Check): `betfair_anker.json` hängte den Senioren-Markt an das Nachwuchsspiel — „Real Madrid U19 v Inter U19" führte `ucl-rma-int-2026-09-08` mit **$294.571**, fünf solcher Fälle in einer Datei, einer davon (`ucl-por-mnc`) an **zwei** Betfair-Spielen gleichzeitig. Der Namens-Score kann das prinzipiell nicht: „Real Madrid U19" vs. „Real Madrid CF" ergibt 2/3, „Inter U19" vs. „Inter Milan" 1/2 — Summe über der Schwelle. Nachwuchs, zweite Mannschaft und Frauenteams teilen fast alle Tokens mit der ersten Elf; wer sie über Ähnlichkeit trennen will, dreht am falschen Ende. → `elf_marker()`/`gleiche_elf()` vergleichen die **Ebene vor dem Score** (U15–U23, II/III, B, W, Reserve, Youth, Jong; fehlende Angabe = erste Mannschaft), Guard `check_poly_markt_gehoert_zur_selben_mannschaft` prüft zusätzlich, dass ein Poly-Markt an höchstens einem Spiel hängt. Gegenprobe an den echten Pools: genau die 5 falschen Joins fallen, 0 richtige. **Merksatz: wo zwei Dinge fast gleich heißen, entscheidet nicht die Ähnlichkeit, sondern das eine Merkmal, das sie unterscheidet.**
21. **Ein Feld an der falschen Stelle gesucht ergibt einen harmlosen Default** (08.09.2026, Übersicht-Check): die Signal-Bilanz las `d.signals && d.signals.minFire || 0`; `pulse.signals` existiert nicht, `minFire: 6` liegt in `pulse.signalBoard` — demselben Objekt, aus dem zwei Zeilen höher die Zeilen kommen. Mit Schwelle 0 galt keine Zeile als dünn: **alle 25 Signale standen gleichwertig, fünf davon mit n ≤ 2 neben n = 130.** Verwandt mit Klasse 12, aber eigener Auslöser — nicht kaputte Daten, sondern ein Pfad, der ins Leere zeigt und dessen `|| 0` genau wie ein legitimer Wert aussieht. → Schwelle aus `b.minFire`; fehlt sie wirklich, sagt die Tafel es (`⚠ keine Feuer-Schwelle im Artefakt`). **Ein `|| 0` hinter einer Objektkette ist kein Default, sondern eine unbeobachtete Annahme.**
22. **Ein Element, das seine Betriebsbedingung verschweigt, liest sich als Befund** (08.09.2026, Uebersicht-Check): Ebene 2 („Wie viele Buecher sind sich einig?") haengt am Tor `Konzentration UND frischer Zufluss UND Quote zieht rein`. „Frischer Zufluss" heisst >= 2.000 EUR zwischen zwei Snapshots im Abstand von ~15 Minuten — gemessen ueber 192 Spiele im Bestand: conc 114, dir=in 10, **inflow 0**, groesster Wert im ganzen Feld 1.178 EUR. Im Ledger tauchten **76 % aller Zeilen erst < 3 h vor Anpfiff** auf (Median 0,7 h). Die Ebene ist also ein Letzte-Stunde-Instrument, schreibt morgens aber „Gerade deckt sich nichts" — ein Satz ueber die WELT, wo ein Satz ueber das WERKZEUG hingehoert. Real Madrid v Inter stand im selben Moment mit 88 % Geldanteil, 102.861 EUR und 7 von 10 Buecherpunkten unsichtbar daneben. → Ebene 0 (`spielzentrale.py`) beantwortet die Frage ohne Bewegungsbedingung. **Merksatz: wenn eine Flaeche systematisch leer ist, ist „nichts da" die unwahrscheinlichste Erklaerung — pruefe zuerst, ob ihre Bedingung ueberhaupt erreichbar ist, und schreibe sie in den Leertext.**
23. **Ein Anker ist keine Stimme** (08.09.2026, beim Bau der Spielzentrale): Die naheliegende Regel fuer „mehrere Quellen sind sich einig" waere „zwei Quellen nennen dieselbe Seite" — und mit Pinnacle als dritter Quelle waere fast jede Zeile einig, weil das Geld fast immer auf dem Favoriten liegt. Diese Uebereinstimmung ist der Normalfall und traegt null Information. Dasselbe gilt fuer einen reinen Poly-Scan-**Preis**: er sieht aus wie eine Quelle, aber niemand hat ihn bezahlt. → Es stimmen nur unabhaengige GELDquellen ab (Betfair, Polymarket, Stake); der Anker kann ausschliesslich bestaetigen oder widersprechen, und der Preis bleibt sichtbar, ohne mitzuzaehlen. **Merksatz: bevor eine Quelle mitzaehlen darf, frage, wie oft sie zufaellig zustimmt — was fast immer zustimmt, belegt nichts.**
24. **Zwei Zuordnungen fuer dieselbe Frage, und die aermere gatet** (08.09.2026, Poly-Durchsicht): In `poly_whale_watch.py` standen `sport_category()` (volle Regex) und `_sport()` (`SOCCER…/LIGA/MLS/EPL/UCL`) in **derselben Datei**. Entschieden hat die aermere: `_pub_ok()` wirft alles raus, was bei ihr auf dem 🎯-Default landet, und `_pub_ok` filtert **beide** Telegram-Kanaele. Gemessen: 75 von 617 offenen Positionen auf 🎯, **davon 55 echter Fussball** (EFL Championship, Ligue 1, Eliteserien, Ligue 2, Brazil Serie A, Allsvenskan, Scottish Premiership). Der Fingerabdruck stand in den Push-Zahlen: `epl` 54, `lal` 33, aber `fl1` 2, `bra`/`sco`/`all` je 0. Erschwerend: jeder ANDERE Public-Filter druckt eine Unterdrueckungszeile (🚫 💭 🤝 ⚔️), dieser nicht — der Verlust war im Log unsichtbar. → `_sport()` haengt nur noch das Emoji an `sport_category()`; 59 der 75 Positionen frei, 0 falsche dazu. **Merksatz: wenn zwei Funktionen dieselbe Frage beantworten, entscheidet irgendwann die schlechtere — und ein Filter ohne Log-Zeile ist ein Filter, den niemand findet.**
25. **Ein Wort, das der eigene Guard widerlegt** (08.09.2026, Poly-Durchsicht): `killer.py._bewiesene_wallets` hatte eine dritte, eigene Sharp-Definition (`n>=8 und Ø CLV > 0`) neben `sharp_gate.is_sharp` und `sharp_gate.sharp_grade`. Sie lieferte **233** Wallets gegen 51 bzw. 19 — und **91 der 233 waren bestaetigte Verlierer**, die groesste mit n=186 und **P&L −$7.775.708**. Das eigene Statusfile meldete es seit Tagen (`proven_wallets_profitable`: „101/238 sind netto-NEGATIV"), gelesen hat es an dieser Stelle niemand. → Delegiert an `sharp_gate`, und zwar an die LOCKERE Kante der Rampe (`grade > 0`), weil am 01.09. gemessen wurde, dass der strengste Schalter die schlechteste Vorwaertsleistung liefert. **Merksatz: wenn eine Flaeche ein Wort wie „bewiesen" benutzt, pruefe, ob ein anderer Teil des Systems dieses Wort bereits widerlegt — der Widerspruch steht meistens schon irgendwo geschrieben.**
26. **Ein Anteil altert, ein Preis nicht** (08.09.2026, Lucas ueber zwei Remis-Pushs): Der Geld-Anteil eines Ausgangs ist kumulierter Umsatz ueber die GANZE Marktlaufzeit, die Quote daneben ist von jetzt. Bei 2:2 in der 70. Minute wird auf das Remis gehandelt; faellt das 3:2, bleibt das Geld in der Statistik stehen und die Quote springt. Gemessen im Feed: „Estoril U23, Min 87, 3:2 — The Draw **86 % Anteil @6,60** (implizit 15 %)" und „Porto U19 v Man City U19, Min 45, 2:0 — The Draw **48 % @14,00** (implizit 7 %)". Von 6 laufenden Spielen genau diese zwei. Ein bestehender Filter (`_pub_incoherent`, >=70 % auf >=3,0) fing beide nicht: Man City lag unter der Anteils-Schwelle, und der Filter lief nur auf dem Public-Pfad, waehrend der Push in Trades kam. → `geld_ist_altbestand()`: in-play, >=40 % Anteil auf >=5,0 Quote, beide Kanaele. **Merksatz: bevor eine Anteils-Zahl etwas belegt, frage, ueber welchen Zeitraum sie summiert — und ob der Preis daneben aus demselben Moment stammt.**
27. **Ein ROI, dessen Vorzeichen an der Preiswahl haengt, ist eine Einstellung** (08.09.2026, Remis-Auswertung): `betfair_draw_tracker` rechnete seine In-Play-Eimer gegen `lastDrawOddInplay` — die LETZTE gesehene In-Play-Quote, also die vom Schlusspfiff (Median @7,80, **Mittel @96,80, Max @1.000**; bei einem 4:2 steht das X bei 1.000). Derselbe Eimer, dieselben 869 Spiele: **−36,9 %** mit dieser Basis, **+24,2 %** mit der Quote beim Gleichstand. Der Code-Kommentar verlangte sogar `firstLevelOdd`; uebergeben wurde die andere. Aus der falschen Zahl war seit dem 13.08. eine Push-Sperre begruendet. Der echte Einstiegspreis (Quote im Moment des Geldzuflusses) wird gar nicht mitgeschrieben. → Beide Raender werden ausgewiesen, `backRoi: None`. **Merksatz: bei jedem ROI zuerst fragen, ob der Preis, gegen den gerechnet wird, zum Einstiegszeitpunkt ueberhaupt handelbar war — ein Preis nach dem Ereignis ist kein Einstieg, sondern ein Rueckblick.**

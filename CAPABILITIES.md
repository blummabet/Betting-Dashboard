...
| `polywallets` | `poly-wallets.js` | **Whale Tracker**, zwei Views: „Edge & Smart-Money" (Edge=Signal, Whales=Veto + Auflösungs-Lücken + interne Fehlbepreisung + Whale-Einstiegsqualität) und **„Liegt das Geld richtig?"** (Geld-Genauigkeit: Brier Geld vs Preis). Einstieg MLS. |
...
- `poly_money_broad.py` — **BREIT über alle Poly-Ligen** (19.07.): alles was Poly anbietet mit Volumen ≥ Schwelle (~7.5k) + Quote ≥ 1.35, aufgelöst über **Polys eigene Settlement** (kein externer Feed nötig). Je Liga: wo hat die Masse mehr recht? Teilt `evaluate` mit money_accuracy. → `poly_money_broad.json` (global) → Wallets-Geld-View „🌐 Alle Poly-Ligen". Fetch/Resolution scharf nur am **Mac-Runner** (manage-mls-poly).

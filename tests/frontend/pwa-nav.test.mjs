...
test('PWA-Tags + Navi-Markup (Bottom-Nav, Sheet, Web-Dropdown) im HTML vorhanden', () => {
  ...
  // Mobile Bottom-Nav (5 Tabs: Home/National/Sharp/Trading/Mehr) + Sheet (9: Betfair, Intl, Poly×2, Pinnacle×Poly, TikTok, Heart, Status, Analyse)
  assert.equal(d.querySelectorAll('.bottom-nav .bn-btn').length, 5, '5 Bottom-Tabs erwartet');
  assert.equal(d.querySelectorAll('.more-sheet .ms-btn').length, 9, '9 Sheet-Einträge erwartet (inkl. Betfair + Intl + Pinnacle×Poly)');
